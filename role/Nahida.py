...
class Nahida(Role):
    ...
    def buff_T1(self, buff_info: BuffInfo, prop: DmgCalc):
        ...
        if setting.label.isdigit():
            ...
            ext_elem_ma = 0
        elem_ma = min(max(prop.elem_mastery, ext_elem_ma), 250)
        setting.state = "最高精通" + str(elem_ma)
        buff_info.buff = Buff(
            dsc="摩耶之殿领域内，基于队伍中最高精通，场上角色精通+50",
    ...
    def buff_T2(self, buff_info: BuffInfo, prop: DmgCalc):
        ...
        buff_info.buff = Buff(
            dsc=f"基于精通超过200点的部分，灭净三业增伤+{(dmg_bonus*100):.1f}%，暴击+{(dmg_bonus*100):.1f}%",
            target="E",
            dmg_bonus=dmg_bonus,
            crit_rate=crit_rate,
        )
    ...
    def skill_E(self, dmg_info: Dmg):
        ...
        scaler1, scaler2 = float(
            self.get_scaler("所闻遍计", self.talents[1].level, "灭净三业伤害")
            .replace("%攻击力", "")
            .replace("%元素精通", "")
            .split("+")
        )
    ...
    def buff_Q(self, buff_info: BuffInfo):
        ...
        match fire:
            ...
            case _:
                scaler = 0
        buff_info.buff = Buff(
            dsc=f"摩耶之殿领域内，基于队伍中元素类型，灭净三业增伤+{scaler}%",
            target="E",
